...
program
  .name("persistent")
  .description("Universal AI workflow bootstrap with persistent context — OpenSpec + Skills.sh + Obsidian")
  .version("0.2.0");
...
program
  ...
  .action(async (opts) => {
    ...
    if (!cfg.obsidianPath) {
      ...
      console.log(chalk.dim("Run: persistent init --obsidian <path>  or  persistent sync --discover"));
      process.exit(1);
    }
...
program
  ...
  .action(async (feature, opts) => {
    if (opts.archive)       await archiveSpec(opts.archive);
      ...
      console.log(chalk.dim('  persistent spec "feature name"  — propose a new spec'));
      console.log(chalk.dim("  persistent spec --validate <slug>  — validate a spec"));
      console.log(chalk.dim("  persistent spec --archive <slug>   — archive + evolve SEED"));
      console.log(chalk.dim("  persistent spec --list             — list active specs"));
      console.log(chalk.dim("  persistent spec --seed             — regenerate SEED.md"));
    }
  });
  ...
  .action(async (opts) => {
    ...
    if (opts.search) {
      ...
      if (installed.length === 0) {
        console.log(chalk.dim("No skills installed. Run: persistent add-skill <id>"));
      } else {
      ...
      console.log(chalk.dim("  persistent skill --create <id>     — create from project"));
      console.log(chalk.dim("  persistent skill --evolve <id>     — evolve with new patterns"));
      console.log(chalk.dim("  persistent skill --update          — update all installed"));
      console.log(chalk.dim("  persistent skill --list            — list installed skills"));
    }
  });
